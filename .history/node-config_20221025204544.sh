...
sudo apt update
sudo apt -y install vim git curl wget kubelet=1.23.8-00 kubeadm=1.23.8-00 kubectl=1.23.8-00
...
--apiserver-cert-extra-sans=$IPADDR  --pod-network-cidr=$POD_CIDR --node-name $NODENAME --ignore-preflight-errors Swap
